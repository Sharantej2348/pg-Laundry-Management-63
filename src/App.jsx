...
const LABEL_BY_ID = Object.fromEntries(MACHINE_DEFS.map((d) => [d.id, d.label]));
...
function mapRow(row) {
  return {
    ...
    note: row.note || null,
    reportedAt: row.reported_at ? new Date(row.reported_at).getTime() : null,
  };
}
...
function useAlarmEngine() {
  ...
  const startTitleFlash = useCallback((label) => {
    ...
    titleFlashRef.current = setInterval(() => {
      document.title = on ? originalTitleRef.current : `⏰ ${label} done!`;
      on = !on;
    }, 1000);
  }, []);
  ...
  const startLoop = useCallback(
    (label) => {
      ...
      autoStopTimeoutRef.current = setTimeout(stopLoop, ALARM_AUTO_STOP_MS);
    },
  ...
  const tryPushNotification = useCallback((label) => {
    try {
      ...
      if (Notification.permission === "granted") {
        new Notification("Laundry done", { body: `${label} has finished washing.` });
      }
    } catch (e) {}
  }, []);
  ...
  return { unlockAudio, startLoop, stopLoop, tryPushNotification, requestPushPermission, testBeep };
}
...
function MachineCard({
  ...
  const { status, label, user, startTime, endTime, note, reportedAt } = machine;
  const who =
    user && (user.name || user.room)
      ? [user.name, user.room ? `Room ${user.room}` : null].filter(Boolean).join(" · ")
      : null;
    ...
    <div className={`lm-card lm-card-${tone}`}>
      ...
      {status === "available" && (
        <>
          ...
          <button className="lm-btn lm-btn-primary" onClick={() => onOpenStart(machine.id)}>
            Use machine
          </button>
          <button className="lm-btn-link" onClick={() => onOpenOutOfOrder(machine.id)}>
            Report a problem
          </button>
        </>
      )}
        ...
        <>
          ...
          <div className="lm-notify-row">
            ...
            >
              ...
              {notifyOn ? "We'll alert you here when it's done" : "Notify me when done"}
            </button>
            {notifyOn && (
              <button className="lm-btn lm-btn-test" onClick={onTestAlarm} title="Test alarm sound">
                🔊
              </button>
            )}
          </div>
          <div className="lm-secondary-row">
            <button className="lm-btn-link" onClick={() => onOpenExtend(machine.id)}>
              Extend time
            </button>
            <button className="lm-btn-link lm-btn-link-danger" onClick={() => onOpenCancel(machine.id)}>
              Cancel wash
            </button>
          </div>
        </>
      ...
      {status === "finished" && (
        <>
          ...
          <div className="lm-meta">
            <div className="lm-meta-row lm-meta-muted">Finished {formatAgo(now - endTime)}</div>
            {who && <div className="lm-meta-row">{who}</div>}
          </div>
          <button className="lm-btn lm-btn-finished" onClick={() => onRemoveClothes(machine.id)}>
            Clothes removed
          </button>
          <button className="lm-btn-link" onClick={() => onOpenOutOfOrder(machine.id)}>
            Report a problem
      ...
      {status === "outOfOrder" && (
        <>
          <p className="lm-card-sub">{note ? note : "Needs attention before it can be used"}</p>
          <div className="lm-meta">
            {reportedAt && <div className="lm-meta-row lm-meta-muted">Reported {formatAgo(now - reportedAt)}</div>}
          </div>
          <button className="lm-btn lm-btn-primary" onClick={() => onOpenMarkWorking(machine.id)}>
...
function StartWashModal({ machine, onCancel, onConfirm, submitting }) {
  ...
  const effectiveMinutes = useCustom ? parseInt(customMinutes, 10) : selectedPreset;
  const isValid =
    Number.isFinite(effectiveMinutes) && effectiveMinutes > 0 && effectiveMinutes <= MAX_CUSTOM_MINUTES;
  ...
  const handleStart = () => {
    ...
    onConfirm({ minutes: effectiveMinutes, name: name.trim(), room: room.trim() });
  };
    ...
    <div className="lm-sheet-backdrop" onClick={onCancel}>
      <div className="lm-sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="lm-sheet-header">
          <h2>{machine.label}</h2>
          <button className="lm-icon-btn" onClick={onCancel} aria-label="Close">
            <X size={20} />
          </button>
        </div>
        ...
        <p className="lm-sheet-label lm-sheet-label-spaced">Who's using this machine? (optional)</p>
        <input
          className="lm-input"
          type="text"
          placeholder="Name"
        ...
        <p className="lm-sheet-hint">🔔 We'll sound an alarm on this device when it's done.</p>
        ...
        <div className="lm-sheet-actions">
          ...
          <button className="lm-btn lm-btn-primary" onClick={handleStart} disabled={!isValid || submitting}>
  ...
  return (
    <div className="lm-sheet-backdrop" onClick={onCancel}>
      <div className="lm-sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="lm-preset-row" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
          {EXTEND_PRESETS.map((mins) => (
            <button key={mins} className="lm-preset-btn" onClick={() => onConfirm(mins)}>
              +{mins}m
            </button>
          ))}
        </div>
        <p className="lm-sheet-hint">New finish time updates for everyone within a second or two.</p>
        <div className="lm-sheet-actions">
          <button className="lm-btn lm-btn-ghost" onClick={onCancel}>
            Never mind
          </button>
    ...
    <div className="lm-sheet-backdrop" onClick={onCancel}>
      <div className="lm-sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="lm-sheet-hint">
          This marks the machine unavailable for everyone until someone sets it back to working.
        </p>
        <div className="lm-sheet-actions">
          <button className="lm-btn lm-btn-ghost" onClick={onCancel}>
            Cancel
          </button>
          <button className="lm-btn lm-btn-primary" onClick={() => onConfirm(note.trim())}>
            Mark out of order
          </button>
        </div>
      </div>
    ...
    <div className="lm-alarm-backdrop" onClick={onCancel}>
      <div className="lm-alarm-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="lm-sheet-actions">
          ...
          <button className={`lm-btn ${danger ? "lm-btn-danger" : "lm-btn-primary"}`} onClick={onConfirm}>
            {confirmLabel}
          </button>
        </div>
      </div>
  ...
  return (
    <div className="lm-alarm-backdrop">
      <div className="lm-alarm-card">
        ...
        <p>Time to grab your clothes before someone else needs the machine.</p>
...
export default function App() {
  ...
  const [notifyPrefs, setNotifyPrefs] = useState(() => readLocal(NOTIFY_PREFS_KEY, {}));
  ...
  const markAlarmed = useCallback((key) => {
    const next = [...alarmedCyclesRef.current, key].slice(-MAX_ALARMED_CYCLES_REMEMBERED);
    ...
    (next) => {
      ...
      if (prev) {
        for (const id of Object.keys(next)) {
          const wasWashing = prev[id] && prev[id].status === "washing";
          const isFinishedNow = next[id] && next[id].status === "finished";
          if (wasWashing && isFinishedNow && notifyPrefsRef.current[id]) {
            const key = cycleKeyFor(next[id]);
            if (!alarmedCyclesRef.current.includes(key)) {
              markAlarmed(key);
              setAlarmQueue((q) => [...q, { id, label: LABEL_BY_ID[id], cycleKey: key }]);
            }
          }
        }
      }
  ...
  useEffect(() => {
    const tick = setInterval(() => {
      ...
      for (const id of Object.keys(current)) {
        const m = current[id];
        if (m.status === "washing" && m.endTime !== null && ts >= m.endTime) {
          supabase.from("machines").update({ status: "finished" }).eq("id", id).eq("status", "washing");
        }
      }
    }, TICK_INTERVAL_MS);
  ...
  const handleToggleNotify = (machineId) => {
    ...
    const next = { ...notifyPrefsRef.current, [machineId]: !notifyPrefsRef.current[machineId] };
  ...
  const handleConfirmStart = async ({ minutes, name, room }) => {
    ...
    if (error || !data || data.length === 0) {
      setClaimError("Someone just claimed this machine. Pick another one.");
    ...
    applyIncoming({ ...machinesRef.current, [modalMachineId]: mapRow(data[0]) });
    setModalMachineId(null);
  };
  ...
  const handleRemoveClothes = (machineId) =>
    setMachineAvailableWithUndo(machineId, `${LABEL_BY_ID[machineId]} marked available.`);
  ...
  const handleUndoRemoveClothes = async () => {
    ...
    await supabase
      ...
      .update({
        ...
        start_time: s.startTime ? new Date(s.startTime).toISOString() : null,
        end_time: s.endTime ? new Date(s.endTime).toISOString() : null,
        note: s.note,
        reported_at: s.reportedAt ? new Date(s.reportedAt).toISOString() : null,
      })
  ...
  const handleOpenCancel = (machineId) => setConfirmDialog({ type: "cancelWash", machineId });
  const handleOpenMarkWorking = (machineId) => setConfirmDialog({ type: "markWorking", machineId });

  const handleConfirmDialogAccept = () => {
    if (!confirmDialog) return;
    const { type, machineId } = confirmDialog;
    if (type === "cancelWash") setMachineAvailableWithUndo(machineId, `${LABEL_BY_ID[machineId]} wash cancelled.`);
  ...
  const handleConfirmExtend = async (minutes) => {
    ...
    const newEndTime = new Date(target.endTime + minutes * 60 * 1000).toISOString();
    await supabase.from("machines").update({ end_time: newEndTime }).eq("id", extendMachineId).eq("status", "washing");
    setExtendMachineId(null);
  };

  const handleOpenOutOfOrder = (machineId) => setOutOfOrderMachineId(machineId);
  ...
  const outOfOrderMachine = outOfOrderMachineId ? machines?.[outOfOrderMachineId] : null;
    ...
    <div className="lm-root">
      <header className="lm-header">
        ...
        <div>
          ...
          <div className={`lm-sync ${connectionStatus === "live" ? "lm-sync-live" : ""}`}>{connectionLabel}</div>
        </div>
      </header>
        ...
        {!machines &&
          MACHINE_DEFS.map((def) => (
            <div className="lm-card lm-card-loading" key={def.id}>
              <div className="lm-skeleton-line" style={{ width: "50%" }} />
              <div className="lm-skeleton-line" style={{ width: "30%" }} />
            </div>
          ))}
        {machines &&
          MACHINE_DEFS.map((def) => (
      ...
      <p className="lm-demo-note">Updates sync live to everyone using this page.</p>
      ...
      {extendMachine && (
        <ExtendModal machine={extendMachine} onCancel={() => setExtendMachineId(null)} onConfirm={handleConfirmExtend} />
      )}
